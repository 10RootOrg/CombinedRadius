...
const RestApiRunModel = async (data, davidServerClose) => {
  const tunnelUrl = `http://${
    data.UrlIpType.toLowerCase() === "private"
      ? ip.address()
      : data.StaticIp
      ? data.StaticIp
      : pubIp
  }:${isExternalServer ? isExternalServer : data.port}`;
  const EmailHost =
  ...
  const totpObj = {
    timestamp: Date.now(),
    digits: data?.TotpObject?.digits || 3,
    algorithm: data?.TotpObject?.algorithm || "SHA-1",
    secret: data?.TotpObject?.secret || "NBSWY3DPEBGXSICOIFWWKICJOMQEA2",
  };

  for (let i = 0; i < serverUseTimeRa[data.port].length; i++) {
    const encoded = base32.encode(serverUseTimeRa[data.port][i]);
    const secret = totpObj.secret + encoded;
    const token = totp(secret, totpObj);
    console.log(`${serverUseTimeRa[data.port][i]} token `, token);
  }
  ...
  if (data.WayOfNotification == "Phone" || data.WayOfNotification == "Both") {
    if (data.TOTPEmail) {
      for (let i = 0; i < data.TOTPEmail.length; i++) {
        ...
        sendTOTPEmail(data, element, EmailHost, totpObj, davidServerClose);
      }
    }
...
async function sendnoti(
  ...
  try {
    const signature = signMassage(
      SignatureMessage[info.port],
      PrivateKey[info.port]
    );
    ...
    const res = await fetch("https://exp.host/--/api/v2/push/send", {
      ...
      body: JSON.stringify({
        ...
        data: {
          message: SignatureMessage[info.port],
          signature: signature,
          phase2url: `${tunnelUrl}/Verify/accept/${token}/${isExternalServer?info.port+"/":""}`,
          nourl: `${tunnelUrl}/Verify/reject/${token}${isExternalServer?"/"+info.port:""}`,
          noVerify: `${tunnelUrl}/Verify/noVerify/${token}${isExternalServer?"/"+info.port:""}`,
          uuid: requestID,
          time: requestTime,
          ...
          fqdn: FQDN,
          TOTP: info.TOTP,
          TOTLength: totpObj.digits,
          location: locationVar[info.port],
          noLocationUrl: `${tunnelUrl}/Verify/wrongLocation/${token}${isExternalServer?"/"+info.port:""}`,
        },
      }),
    });
    ...
    davidServerClose(5);
    isRestServerOn ? clearTimeout(timeOutRa[info.port]) : "";
...
async function sendTOTPEmail(
  ...
  try {
    let emailText = `\n`;
    for (let i = 0; i < serverUseTimeRa[info.port].length; i++) {
      const encoded = base32.encode(serverUseTimeRa[info.port][i]);
      const secret = totpObj.secret + encoded;
      const token = totp(secret, totpObj);
      emailText =
        emailText + ` ${token} For ${serverUseTimeRa[info.port][i]}  \n`;
    }
...
async function sendEmail(
  ...
  try {
    ...
    if (email !== "") {
      ...
      const tot = info.TOTP ? totp(secret, totpObj) : 111;
      ...
      let action = await transporter.sendMail({
        ...
        <h1 style="font-size: 14px; font-family: Arial;color:  #8b8b8b;" >Conditional Access Request:</h1>
            ...
            <a font href="${tunnelUrl}/Verify/accept/${token}/${isExternalServer?info.port+"/":""}${tot}"><button style="background-color:#1EBE54; font-size: 14px; padding: 5px 25px ; color: #ffffff; border: 0; border-radius: 5px;  width: 124px; margin-right: 8px;"> Approve &#10003;</button></a>       
            <a font href="${tunnelUrl}/Verify/reject/${token}${isExternalServer?"/"+info.port:""}"><button style="background-color:#E13738; font-size: 14px; padding: 5px 25px ; color: #ffffff; border: 0; border-radius: 5px;width: 124px"> Deny &#10006;</button></a>  <br>   
            </div>`,
      });
      info.LOG
        ? console.log(` Email log ${
